...
let playerShootTimer = new Timer(1000 / PLAYER_FIRE_RATE)
  ...
  ctx.fillText("Coins: " + playerCoins, 2, 30)
	...
	if (playerHealth === 0) {
	  ...
	  ctx.font = "40px sans-serif"
		const gameOverTextWidth = ctx.measureText(text).width
		ctx.fillText(
			text,
	  ...
	  const playButtonTextWidth = ctx.measureText(playButtonText).width
	  ctx.fillText(
			playButtonText,
			canvas.width / 2 - playButtonTextWidth / 2,
			canvas.height / 2 + 40
		)
	}
